...
| Metropolitan Opera | `met_opera` | 実装済み（スクリーンショットを元に設計、ユニットテスト済み。API調査はできず現在表示中の月のみ取得） |

5館すべて実装済みですが、**いずれもスクリーンショット（実際のHTML/DOMではなく見た目）を元にした
設計で、実サイトに対する動作検証はできていません**（理由は次項）。パーサーが実際の構造と
ズレていた場合は空リストを返すだけで実行自体は失敗しませんが、`No performances extracted for ...`
という警告がログに出ます。その場合は該当パーサーの調整が必要です。
